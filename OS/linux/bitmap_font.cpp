...
{
	return (struct retroTextWindow **) sys_public_alloc_clear( sizeof( struct retroTextWindow * ) * n );
}

struct retroTextWindow *new_text_window ( int id )
{
	struct retroTextWindow *textWindow = (struct retroTextWindow *) sys_public_alloc_clear( sizeof( struct retroTextWindow  ) );
...
void delTextWindow( struct retroScreen *screen, struct retroTextWindow *window )
{
	if (screen)
	{
		...
		{
			if (screen -> textWindows) sys_free( screen -> textWindows );
			screen -> textWindows = NULL;
		}
	}
}
